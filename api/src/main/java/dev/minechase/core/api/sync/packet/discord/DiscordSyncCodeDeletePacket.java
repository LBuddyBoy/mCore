...
@AllArgsConstructor
@Getter
public class DiscordSyncCodeDeletePacket extends ServerResponsePacket {

    private final SyncCode code;
    private final String executeServer;

    public DiscordSyncCodeDeletePacket(SyncCode code) {
        this.code = code;
    ...
    public void onReceiveExecuteServer() {
        CoreAPI.getInstance().getDiscordSyncHandler().removeCode(this.code);
        CoreAPI.getInstance().getDiscordSyncHandler().deleteCode(this.code, true);
    }

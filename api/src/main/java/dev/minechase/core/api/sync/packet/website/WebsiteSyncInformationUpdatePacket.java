...
@AllArgsConstructor
@Getter
public class WebsiteSyncInformationUpdatePacket extends ServerResponsePacket {

    private final WebsiteSyncInformation information;
    private final String executeServer;

    public WebsiteSyncInformationUpdatePacket(WebsiteSyncInformation information) {
    ...
    public void onReceiveExecuteServer() {
        CoreAPI.getInstance().getWebsiteSyncHandler().updateInfo(this.information);
        CoreAPI.getInstance().getWebsiteSyncHandler().saveInfo(this.information, true);
    }
